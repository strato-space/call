...
def discover_agent_repo() -> Path:
    ...
    try:
        ...
        raise FileNotFoundError(
            "Agent repository not found. Set AGENT_REPO env to its path."
        )
...
def load_projects_index(repo: Path | None = None) -> list[str]:
    ...
    try:
        ...
        return [
            r.get("project") for r in rows if isinstance(r, dict) and r.get("project")
        ]
    except Exception:
        return []
...
def scan_project_agents(project_dir) -> list[dict]:
    ...
    def _from_md(p: _Path, *, default_name: str) -> tuple[str, list[str], list[str]]:
        ...
        name = str(
            meta.get("id") or meta.get("name") or meta.get("title") or default_name
        )
        aliases = (
            [str(a).strip() for a in (meta.get("aliases") or [])]
            ...
            if amd.exists():
                ...
                if ay.exists():
                    try:
                        ...
                        name = str(y.get("id") or y.get("name") or child.name)
                        aliases = (
                            [str(a).strip() for a in (y.get("aliases") or [])]
                            if isinstance(y.get("aliases"), list)
                            else []
                        )
                        pv = y.get("prompts") or []
...
def _ensure_indices(rep: Path) -> None:
    ...
    for base in projects:
        ...
        aliases_map = {
            name: aliases for name, (_, aliases) in scanned.items() if aliases
        }
        # Enrichment from project.yaml when directory scan finds nothing
        if not agents_map:
            proj_yaml = base / "project.yaml"
            fallback_yaml = (
                (rep / "AgentFab" / "agent.yaml") if base.name == "AgentFab" else None
            )
                ...
                try:
                    ...
                    if isinstance(section, dict):
                        ...
                        for nm, spec in section.items():
                            agents_map[str(nm)] = (
                                "" if not isinstance(spec, str) else str(spec or "")
                            )
                        ...
                        for nm, spec in section.items():
                            if isinstance(spec, dict):
                                ...
                                if isinstance(av, list) and av:
                                    aliases_map[str(nm)] = [
                                        str(a) for a in av if str(a).strip()
                                    ]
                except Exception:
                    pass
        content = {
...
def discover_agent_yaml(agent_name: str, project: str | None = None) -> Path | None:
    ...
    debug_print(
        "[discovery] discover_agent_yaml (db-only):",
        f"agent={name}",
...
def _choose_best_prompt(
    paths: list[Path], *, project: str | None, agent: str | None
) -> Path | None:
    ...
    def key(p: Path):
        ...
        agent_match = (
            (agent or "").lower().replace(" ", "") == m_agent.lower().replace(" ", "")
            if agent
            else False
        )
        return (
            (
                0
                if (proj_match and agent_match)
                else 1 if proj_match else 2 if agent_match else 3
            ),
            prio.get(m_proj, 9),
            m_agent.lower(),
            p.name.lower(),
...
def resolve_prompt(
    ...
    *,
    ...
    try:
        ...
        rows = _repo_db.list_prompts(
            project=(project or None),
            agent=(agent or None),
            prompt=str(name).strip(),
            state=("ready" if prefer_ready else None),
        )
    except Exception:
        rows = []
    if len(rows) != 1:
