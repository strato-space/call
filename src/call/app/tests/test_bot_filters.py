...
def test_parse_basic_bare_tokens():
    ...
    proj, agent, prompt, target = parse_prompts_filters(
        "/prompts_ready UxFab 13-*", command="/prompts_ready", default_project=None
    )
    assert proj == "UxFab"
...
def test_parse_agent_shorthand_and_kwargs():
    ...
    proj, agent, prompt, target = parse_prompts_filters(
        text, command="/prompts_ready", default_project=None
    )
    assert proj == "UxFab"
...
def test_parse_long_opts_and_target():
    text = (
        "/prompts --project FanFab --agent Stratoslav --prompt 33-* --target r:FanFab/*"
    )
    proj, agent, prompt, target, state = parse_prompts_and_state(
        text, command="/prompts", default_project=None
    )
    assert proj == "FanFab"
...
def test_parse_state_variants():
    ...
    proj2, agent2, prompt2, target2, state2 = parse_prompts_and_state(
        text2, command="/prompts", default_project="AgentFab"
    )
