...
def list_missing() -> Tuple[List[Path], List[Path]]:
    ...
    cmd = [
        sys.executable,
        str(ROOT / "call" / "scripts" / "list_missing_goal.py"),
        "--repos",
        "agent",
        "prompt",
    ]
...
def normalize_file_via_llm(
    path: Path, *, project_name: str = "AgentFab", agent_name: str = "StratoFormater"
) -> Optional[str]:
...
def main():
    ap = argparse.ArgumentParser(
        description="Normalize files missing goal via StratoFormater (LLM)"
    )
    ap.add_argument(
        "--limit",
        type=int,
        default=10,
        help="Limit number of files to process (missing-goal only)",
    )
    ap.add_argument(
        "--apply", action="store_true", help="Write changes back to files (in-place)"
    )
    ap.add_argument(
        "--include-no-metadata",
        action="store_true",
        help="Also process files without METADATA (LLM will create sections)",
    )
    args = ap.parse_args()
    ...
    for p in targets:
        ...
        if out:
            ...
            try:
                ...
                res_probe = call_api.call(
                    project="AgentFab", agent="StratoFormater", input=payload_probe
                )
