...
- 18:20 Document tool exposure controls and fix `mcp_config.yaml` path references; add agent-composition note to docs index. (files: `docs/mcp_config.md`, `docs/when-agents-call-agents.md`, `README.md`, `AGENTS.md`)
...
- **MCP Singleton Lifecycle:** Changed MCP servers to singleton pattern. Servers created once at initialization in `_validate_and_cache_mcp_config()`, cached in `_MCP_SERVERS_CACHE`, and reused across all calls. Added `cleanup_mcp_servers()` for proper shutdown cleanup. Performance improvement by eliminating repeated server creation/destruction. (`app/call.py`, `AGENTS.md`)
- **MCP Config:** Renamed `_initialize_mcp_servers_once()` → `_validate_and_cache_mcp_config()`. Now creates and caches both config and server instances. (`app/call.py`, `app/tests/test_mcp_config_yaml.py`)
...
- **Tests:** Fixed 12 failing tests - updated MCP hook mocks for singleton pattern, fixed @ prefix normalization tests, updated CLI project resolution test, added group chat type attribute. All 12 tests now passing. (`app/tests/test_mcp_hook_integrity.py`, `app/tests/test_telegram_bot_handlers.py`, `app/tests/test_cli_prompts_and_exec.py`, `app/tests/test_telegram_bot_logging.py`)
- **Documentation:** Updated `AGENTS.md` with MCP Lifecycle Singleton Pattern section detailing initialization, server lifecycle, cleanup, and development rules. Added "Automatic JSON-to-YAML Conversion" section to `docs/formatting.md`. Function names updated throughout docs.
...
- **MCP Hook:** Simplified debug print formatting for MCP tool results. Escape sequences (`\n`, `\t`) are unescaped before YAML formatting so multiline strings render cleanly with literal block scalars (`|`). (`app/call.py`)
