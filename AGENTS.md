...
- `mcp_config.sample.yaml` (public template) plus `mcp_config.yaml` / `mcp_config.json` (local overrides) — declarative configuration for external MCP services (filesystem, sequential thinking, Google Sheets, voice, etc.). Supports per-server tool exposure controls via `tools` allowlists and `blocked_tool_names`/`blocked_tools` blocklists. Keep comments synchronized with actual presets and adjust CLI/docs when toggling defaults.
