...
async def _build_payload(name, main_text, reply_text=None, with_document=False):
    ...
    reply = DummyMessage(text=reply_text or "", document=(SimpleNamespace(file_id="doc1") if with_document else None))
    ...
    arg, payload = await build_input_payload_from_reply(name, main_text or "", update, ctx)
    try:
...
async def test_payload_reply_with_document(monkeypatch):
    ...
    arg, payload, parsed = await _build_payload("AgentX", "", reply_text="", with_document=True)
    assert isinstance(parsed, dict)
